...
def render_cube(
    cube_path="data/cube.obj", image_size=256, color=[0.1, 1.0 , 0.2], device=None, camera_rotation = None, camera_translation = None
):
    ...
    vertices, faces = load_mesh(cube_path)
    ...
    # Place a point light in front of the cube.
...
def get_camera_poses(distance=7.0,elevation=0.0,number_of_views=36):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cube_path", type=str, default="data/cube.obj")
    parser.add_argument("--output_path", type=str, default="images/cube_render_360.jpg")
    ...
    camera_poses = get_camera_poses(distance=7.0,elevation=0.0,number_of_views=frames)
    for pose in camera_poses:
        rend = render_cube(cube_path=args.cube_path, image_size=args.image_size,camera_rotation=pose[0],camera_translation=pose[1])
        image = Image.fromarray((rend * 255).astype(np.uint8))
        my_images.append(np.array(image))

    

    imageio.mimsave('images/cube_render_360.gif', my_images, duration=0.2)
